...
void FBackgroundBlurPostProcessor::UpsampleRect(FRHICommandListImmediate& RHICmdList, IRendererModule& RendererModule, const FBlurRectParams& Params, const FPostProcessRectParams& RectParams, const FIntPoint& DownsampleSize, FSamplerStateRHIRef& Sampler)
{
	...
	FVector2D MaskTransform = (Params.MaskTextureChannel == 3 && Params.bMaskRevertAlpha) ? FVector2D(-1.0f, 1.0f) : FVector2D(1.0f, 0.0f);
	...
	RHICmdList.BeginRenderPass(RPInfo, TEXT("UpsampleRect"));
	{
		...
		PixelShader->SetSourceUVBounds(RHICmdList, FVector4(FVector2D::ZeroVector, FVector2D(SourceSizeU, SourceSizeV)));
		...
		PixelShader->SetUVBounds(RHICmdList, FVector4(FVector2D::ZeroVector, FVector2D(SizeU, SizeV)));
...
int32 FBackgroundBlurPostProcessor::ComputeBlurWeights(int32 KernelSize, float StdDev, TArray<FVector4>& OutWeightsAndOffsets)
{
