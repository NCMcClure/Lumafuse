...
class SOCKETCLIENT_API USocketClientPluginUDPClient : public UObject
{
...
public:
	...
	void init(USocketClientBPLibrary* socketClientLibP, FString domain, ESocketClientIPType ipType, int32 port, EReceiveFilterClient receiveFilter, FString clientConnectionID, int32 maxPacketSize = 65507);
...
class SOCKETCLIENT_API FServerUDPSendMessageThread : public FRunnable {
...
class SOCKETCLIENT_API FServerUDPConnectionThread : public FRunnable {

public:

	FServerUDPConnectionThread(USocketClientPluginUDPClient* udpClientP, USocketClientBPLibrary* socketClientP, FString ipP, int32 portP, ESocketClientIPType ipTypeP) :
		udpClient(udpClientP),
		socketClient(socketClientP),
		ipGlobal(ipP),
		portGlobal(portP),
