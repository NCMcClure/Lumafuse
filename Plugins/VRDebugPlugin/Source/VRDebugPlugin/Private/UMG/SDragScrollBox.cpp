...
SDragScrollBox::FSlot::FSlotArguments SDragScrollBox::Slot()
{
	return FSlot::FSlotArguments(MakeUnique<FSlot>());
}

PRAGMA_DISABLE_DEPRECATION_WARNINGS
void SDragScrollPanel::Construct(const FArguments& InArgs, const TArray<SDragScrollBox::FSlot*>& InSlots)
{
	PhysicalOffset = 0;
...
void SDragScrollPanel::Construct(const FArguments& InArgs, TArray<SDragScrollBox::FSlot::FSlotArguments> InSlots)
{
	PhysicalOffset = 0;
...
void SDragScrollPanel::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	{
		const SDragScrollBox::FSlot& ThisSlot = Children[SlotIndex];
...
FVector2D SDragScrollPanel::ComputeDesiredSize(float) const
{
...
float SDragScrollPanel::ArrangeChildVerticalAndReturnOffset(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, const SDragScrollBox::FSlot& ThisSlot, float CurChildOffset) const
{
...
float SDragScrollPanel::ArrangeChildHorizontalAndReturnOffset(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, const SDragScrollBox::FSlot& ThisSlot, float CurChildOffset) const
{
	const FMargin& ThisPadding = ThisSlot.GetPadding();
	const FVector2D& WidgetDesiredSize = ThisSlot.GetWidget()->GetDesiredSize();
...
SDragScrollBox::SDragScrollBox()
{
	VerticalScrollBarSlot = nullptr;
	bClippingProxy = true;
}

void SDragScrollBox::Construct( const FArguments& InArgs )
{
	check(InArgs._Style);
	...
	if (InArgs._ExternalScrollbar.IsValid())
	{
		...
		ScrollBar->SetOnUserScrolled(FOnUserScrolled::CreateSP(this, &SDragScrollBox::ScrollBar_OnUserScrolled));
	...
	SAssignNew(ScrollPanel, SDragScrollPanel, MoveTemp(const_cast<TArray<FSlot::FSlotArguments>&>(InArgs._Slots)))
...
void SDragScrollBox::OnClippingChanged()
{
	ScrollPanel->SetClipping(Clipping);
}

TSharedPtr<SScrollBar> SDragScrollBox::ConstructScrollBar()
{
	return TSharedPtr<SScrollBar>(SNew(SScrollBar)
		.Style(ScrollBarStyle)
		.Orientation(Orientation)
		.OnUserScrolled(this, &SDragScrollBox::ScrollBar_OnUserScrolled));
}

void SDragScrollBox::ConstructVerticalLayout()
{
	...
	[
		...
		[
			...
			[
				...
				.ColorAndOpacity(this, &SDragScrollBox::GetStartShadowOpacity)
			...
			[
				...
				SNew(SImage)
				.Visibility(EVisibility::HitTestInvisible)
				.ColorAndOpacity(this, &SDragScrollBox::GetEndShadowOpacity)
...
void SDragScrollBox::ConstructHorizontalLayout()
{
...
SDragScrollBox::FScopedWidgetSlotArguments SDragScrollBox::AddSlot()
{
...
void SDragScrollBox::RemoveSlot( const TSharedRef<SWidget>& WidgetToRemove )
{
	ScrollPanel->Children.Remove(WidgetToRemove);
}

void SDragScrollBox::ClearChildren()
{
	ScrollPanel->Children.Empty();
}

bool SDragScrollBox::IsRightClickScrolling() const
{
...
float SDragScrollBox::GetScrollOffset() const
{
	return DesiredScrollOffset;
}

float SDragScrollBox::GetScrollOffsetOfEnd() const
{
...
float SDragScrollBox::GetViewFraction() const
{
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
...
float SDragScrollBox::GetViewOffsetFraction() const
{
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
...
void SDragScrollBox::SetScrollOffset( float NewScrollOffset )
{
...
void SDragScrollBox::ScrollToStart()
{
	SetScrollOffset(0);
}

void SDragScrollBox::ScrollToEnd()
{
	bScrollToEnd = true;
...
void SDragScrollBox::ScrollDescendantIntoView(const TSharedPtr<SWidget>& WidgetToScrollIntoView, bool InAnimateScroll, EDescendantScrollDestination InDestination, float InScrollPadding)
{
...
bool SDragScrollBox::InternalScrollDescendantIntoView(const FGeometry& MyGeometry, const TSharedPtr<SWidget>& WidgetToFind, bool InAnimateScroll, EDescendantScrollDestination InDestination, float InScrollPadding)
{
...
EOrientation SDragScrollBox::GetOrientation()
{
	return Orientation;
}

void SDragScrollBox::SetNavigationDestination(const EDescendantScrollDestination NewNavigationDestination)
{
	NavigationDestination = NewNavigationDestination;
}

void SDragScrollBox::SetConsumeMouseWheel(EConsumeMouseWheel NewConsumeMouseWheel)
{
	ConsumeMouseWheel = NewConsumeMouseWheel;
}

void SDragScrollBox::SetOrientation(EOrientation InOrientation)
{
...
void SDragScrollBox::SetScrollBarVisibility(EVisibility InVisibility)
{
	ScrollBar->SetUserVisibility(InVisibility);
}

void SDragScrollBox::SetScrollBarAlwaysVisible(bool InAlwaysVisible)
{
	ScrollBar->SetScrollBarAlwaysVisible(InAlwaysVisible);
}

void SDragScrollBox::SetScrollBarTrackAlwaysVisible(bool InAlwaysVisible)
{
	ScrollBar->SetScrollBarTrackAlwaysVisible(InAlwaysVisible);
}

void SDragScrollBox::SetScrollBarThickness(FVector2D InThickness)
{
	ScrollBar->SetThickness(InThickness);
}

void SDragScrollBox::SetScrollBarPadding(const FMargin& InPadding)
{
...
void SDragScrollBox::SetScrollBarRightClickDragAllowed(bool bIsAllowed)
{
	bAllowsRightClickDragScrolling = bIsAllowed;
}

EActiveTimerReturnType SDragScrollBox::UpdateInertialScroll(double InCurrentTime, float InDeltaTime)
{
...
void SDragScrollBox::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
bool SDragScrollBox::ComputeVolatility() const
{
	return bIsScrolling || IsRightClickScrolling();
}

FReply SDragScrollBox::OnPreviewMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SDragScrollBox::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	{
		if ( MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton && ScrollBar->IsNeeded()  && bAllowsRightClickDragScrolling)
		{
			AmountScrolledWhileRightMouseDown = 0;
...
FReply SDragScrollBox::OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if ( MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton && bAllowsRightClickDragScrolling)
	{
...
FReply SDragScrollBox::OnMouseMove( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	{
		if ( MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton)  && bAllowsRightClickDragScrolling)
		{
...
void SDragScrollBox::OnMouseEnter( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if ( MouseEvent.IsTouchEvent() )
	{
		if ( !bFingerOwningTouchInteraction.IsSet() )
...
void SDragScrollBox::OnMouseLeave( const FPointerEvent& MouseEvent )
{
	if ( HasMouseCapture() == false )
	{
...
FReply SDragScrollBox::OnMouseWheel( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
bool SDragScrollBox::ScrollBy(const FGeometry& AllottedGeometry, float LocalScrollAmount, EAllowOverscroll Overscrolling, bool InAnimateScroll)
{
	Invalidate(EInvalidateWidget::LayoutAndVolatility);
...
FCursorReply SDragScrollBox::OnCursorQuery( const FGeometry& MyGeometry, const FPointerEvent& CursorEvent ) const
{
	if ( IsRightClickScrolling() )
	{
...
FReply SDragScrollBox::OnTouchEnded(const FGeometry& MyGeometry, const FPointerEvent& InTouchEvent)
{
	CachedGeometry = MyGeometry;
...
void SDragScrollBox::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
...
FNavigationReply SDragScrollBox::OnNavigation(const FGeometry& MyGeometry, const FNavigationEvent& InNavigationEvent)
{
	...
	TPanelChildren<SDragScrollBox::FSlot>& Children = ScrollPanel->Children;
	for ( int32 SlotIndex=0; SlotIndex < Children.Num(); ++SlotIndex )
	{
...
void SDragScrollBox::OnFocusChanging(const FWeakWidgetPath& PreviousFocusPath, const FWidgetPath& NewWidgetPath, const FFocusEvent& InFocusEvent)
{
...
TSharedPtr<SWidget> SDragScrollBox::GetKeyboardFocusableWidget(TSharedPtr<SWidget> InWidget)
{
...
int32 SDragScrollBox::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
...
void SDragScrollBox::ScrollBar_OnUserScrolled( float InScrollOffsetFraction )
{
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
...
FSlateColor SDragScrollBox::GetStartShadowOpacity() const
{
...
FSlateColor SDragScrollBox::GetEndShadowOpacity() const
{
...
bool SDragScrollBox::CanUseInertialScroll(float ScrollAmount) const
{
...
EAllowOverscroll SDragScrollBox::GetAllowOverscroll() const
{
	return AllowOverscroll;
}

void SDragScrollBox::SetAllowOverscroll(EAllowOverscroll NewAllowOverscroll)
{
...
void SDragScrollBox::SetAnimateWheelScrolling(bool bInAnimateWheelScrolling)
{
	bAnimateWheelScrolling = bInAnimateWheelScrolling;
}

void SDragScrollBox::SetWheelScrollMultiplier(float NewWheelScrollMultiplier)
{
	WheelScrollMultiplier = NewWheelScrollMultiplier;
}

void SDragScrollBox::SetScrollWhenFocusChanges(EScrollWhenFocusChanges NewScrollWhenFocusChanges)
{
	ScrollWhenFocusChanges = NewScrollWhenFocusChanges;
}

void SDragScrollBox::BeginInertialScrolling()
{
	if ( !UpdateInertialScrollHandle.IsValid() )
	{
		bIsScrolling = true;
		bIsScrollingActiveTimerRegistered = true;
		UpdateInertialScrollHandle = RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SDragScrollBox::UpdateInertialScroll));
		Invalidate(EInvalidateWidget::LayoutAndVolatility);
	}
}

void SDragScrollBox::EndInertialScrolling()
{
	bIsScrolling = false;
	bIsScrollingActiveTimerRegistered = false;
	Invalidate(EInvalidateWidget::LayoutAndVolatility);
...
SDragScrollBox::FSlot& SDragScrollBox::Slot()
{
	return *(new SDragScrollBox::FSlot());
}
...
#if ENGINE_MAJOR_VERSION == 5
	SAssignNew(ScrollPanel, SDragScrollPanel)
		.Clipping(InArgs._Clipping)
		.Orientation(Orientation);
#else
	SAssignNew(ScrollPanel, SDragScrollPanel, InArgs.Slots)
		.Clipping(InArgs._Clipping)
		.Orientation(Orientation);
#endif
...
/** Adds a slot to SDragScrollBox */
SDragScrollBox::FSlot& SDragScrollBox::AddSlot()
{
	SDragScrollBox::FSlot& NewSlot = *new SDragScrollBox::FSlot();
